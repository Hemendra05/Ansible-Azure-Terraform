...
resource "azurerm_linux_virtual_machine" "virtualMachine" {
  ...
  provisioner "file" {
    source      = "ansible.cfg"
    destination = "/home/hemendra/ansible.cfg"
    connection {
      type     = "ssh"
  ...
  provisioner "remote-exec" {
    ...
    inline = [
      ...
      "sudo cp /home/hemendra/ansible.cfg   /etc/ansible/ansible.cfg",
      "sudo yum install java-1.8.0-openjdk-devel -y",
      "sudo yum install git -y",
      "sudo pip3 install boto boto3",
      "sudo touch /etc/ansible/inventory",
